...
def upsample_bilinear2d_vec(
    ...
    v1 = input[:, :, x_floor.to(torch.int64), :][:, :, :, y_floor.to(torch.int64)]
    v2 = input[:, :, x_ceil.to(torch.int64), :][:, :, :, y_floor.to(torch.int64)]
    v3 = input[:, :, x_floor.to(torch.int64), :][:, :, :, y_ceil.to(torch.int64)]
    v4 = input[:, :, x_ceil.to(torch.int64), :][:, :, :, y_ceil.to(torch.int64)]
    q1 = torch.mul(v1, x_ceil_view - x_view) + torch.mul(v2, x_view - x_floor_view)
    q2 = torch.mul(v3, x_ceil_view - x_view) + torch.mul(v4, x_view - x_floor_view)
    result = torch.mul(q1, y_ceil_view - y_view) + torch.mul(q2, y_view - y_floor_view)

    # When (x_ceil == x_floor) and (y_ceil == y_floor).
    result_cond1 = input[:, :, x.to(torch.int64), :][:, :, :, y.to(torch.int64)]
    ...
    result = torch.where(torch.eq(x_ceil_view, x_floor_view), result_cond2, result)
    result = torch.where(torch.eq(y_ceil_view, y_floor_view), result_cond3, result)
    result = torch.where(
        torch.logical_and(
            torch.eq(x_ceil_view, x_floor_view), torch.eq(y_ceil_view, y_floor_view)
        ),
        result_cond1,
        result,
    )
